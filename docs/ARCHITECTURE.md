...
| 2026-07-06 | Monday live finding #3 (staging.1) — CRITICAL false CLOSE: just-placed positions marked TARGET HIT/EXITED with fabricated ~target P&L. Cause: close-detectors treated "Forever/GTT gone + not held" as an exit even with ZERO sell fills — on a fresh position that's broker-state lag (new Forever not listed yet; fresh CNC buy not in holdings). Fix (both legacy detectors + engine): a close with NO sell fill requires clearly-not-fresh (>12h) + non-empty list + persisted grace; a real SELL fill still closes immediately. Plus `reopenFalselyClosedPositions` recovery: an estimated close the broker shows still held/protected is auto-RE-OPENED (boot + 4-min, any hour) | suite green (54) |
